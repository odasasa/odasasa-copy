...
const ForgotPassword = ({
    ...
    const handleSubmit = async (values: any) => {
        ...
        try {
            const data = await (await fetch("/api/user/forgot-password", { body: JSON.stringify(values), method: "POST", headers: { "Content-Type": "application/json" } })).json()
    ...
    const inputCommonClasses = 'mb-3  py-2 rounded-sm w-full outline outline-2 outline-auth-border_color'
    return (
     ...
     text-auth-gray border border-solid  border-auth-border_color`, className)}>
                ...
                <div className='w-full p-[20px] h-[111px] flex flex-col  border-solid border-b border-auth-border_color'>
                    <div className='text-center ' >
                        ...
                        <Typography variant={'p'} className=' text-base text-auth-gray font-bold '>Please enter your user information.</Typography>
                ...
                <div className='w-full h-[200px] flex flex-col justify-center items-center px-3  border-solid border-b border-auth-border_color'>
                    <Typography variant={'p'} className=' text-base text-auth-gray font-normal py-4'>Do not worry, we will send you an email to reset your password.</Typography>


                    {/* <div className='px-3 w-full'> */}
                        <Input  name={'email'} label={"Enter Your Email"} type={"email"} className={inputCommonClasses} />
                        <Button className="mx-3 bg-auth-blue hover:auth-hover_blue text-white w-full  py-2" type='submit' >Reset Password</Button>
