...
const ProfileSettings = ({ handleRefresh }: any) => {
    ...
    return (
        <UniversalFormikForm
            handleSubmit={(values) => updateProfile(values, () => { setData({ ...globalData, user: values }); LocalStorageManager.set('user', values); handleRefresh() })}
        ...
        >
            {
                signupFormFields.filter(field => !field.name.toLowerCase().includes('password')).map((field: { name: string, type: string, label: string }, indx: number) =>
                    ['vendor', "businesscode"].includes(field.name.toLocaleLowerCase()) ? <Input key={indx} {...{ ...field, disabled: true }} labeled={true} /> : <Input key={indx} {...field} labeled={true} />)
            }

            <div className="w-full my-3 flex items-center justify-center">
                <Button type="submit" className="w-[96%] mx-auto bg-product-blue text-xl font-bold" onClick={() => console.log("hello")}>Update Info</Button>
